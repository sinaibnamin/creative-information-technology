...
class ProductController extends Controller
{
    ...
    public function save_product(Request $request)
    {
        ...
        $request->validate([
            ...
            $re_plce = preg_replace('/\\s+/', "-", trim(preg_replace('/[^A-Za-z]/', " ", $productName)));
             $directory ='public/product_image/';
             $img = Image::make($image)->resize(300, 200)->save($directory.$re_plce.'.jpg');
            ...
            //  $product->product_image          = $re_plce.'.jpg';
            ...
            if ($request->cat_level_one) {
              ...
              if ($request->cat_level_two) {
                 $product->sub_sub_cat_id      = $request->cat_level_two;
              }
            ...
            $product->status              = $request->home_page;
        }
             $product->save();
        ...
        // $lastProduct->product_image          = $re_plce.$lastProduct->id.'.jpg';
        $lastProduct->save();
    ...
    {
        $product_name="";$category="";$subcategory="";$model_number="";
        ...
        if(!empty($request->get('model_number'))){
            $model_number   = $request->get('model_number');
        }

         $products = Product::query()
            ->where(function ($filter) use ($product_name,$category,$subcategory,$model_number) {
                ...
                if (!empty($model_number))
                    $filter->where('model_number', '=',$model_number);
            }) ->get();
        ...
        //             ->orWhere('subcategory_id',$subcategory)
        //             ->orWhere('model_number',$model_number)
        //             ->get();
    ...
    public function edit_product($id)
    {
        ...
        $subSubCategories   = Category::where('root_id','!=',0)->get();
        $brands          = Brand::all();
    ...
    public function update_product(Request $request)
    {
        ...
        if ($image) {
            ...
            $img = Image::make($image)->resize(300, 200)->save($directory . $imageName);
